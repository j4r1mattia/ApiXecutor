 ...
 * @returns {Array} An array of valid collection paths.
...
export async function getCollections(dir) {
  ...
  try {
    ...
    if (pathType === 'directory') {
      ...
      return filteredValidCollections(allFiles);
...
/**
 * Filters a list of file paths to obtain only valid collection paths.
 * @param {Array} files - An array of file paths.
 * @returns {Array} An array of valid collection paths.
 */
function filteredValidCollections(files) {
  return files.filter(isValidCollection);
}
